...
def encoding_network(X, hidden_layer_neurons, latent_dim, reg_coef):
    ...
    std_vector = tf.nn.softplus(GuassianParameters[: , latent_dim: ])
    KL_divergence = 0.5 * tf.reduce_sum(1 + tf.log(tf.square(std_vector)) - tf.square(std_vector) - tf.square(mean_vector))

    # reparameterization trick
    sampled_z = tf.multiply(std_vector, tf.random_normal(mean_vector.shape)) + mean_vector
    return KL_divergence, sampled_z

def decoding_network(X, z, hidden_layer_neurons, reg_coef):
    r"""This network decodes z into X.
        ...
        for epoch in range(FLAGS.num_epoch):
            if epoch % 5 == 0:
                if epoch ==0:
                    ...
                    # Note that _generated_X should be "sigmoided" before mapped into a picure.
